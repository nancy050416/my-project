...
一个基于 Spring AI 的 AI Agent 应用实践项目，围绕「大模型对话 + 工具调用 + RAG 知识库 + 实时执行反馈」构建。项目目标不是只做一个普通聊天框，而是实现一个可以根据上下文进行多轮决策、调用外部工具、检索知识库并把执行过程实时推送给前端的智能体系统。
...
DeepSeek / ZhipuAI ChatModel
Ollama bge-m3 embedding
Markdown 文档解析
向量相似度检索
核心功能
1. Agent Think-Execute Loop
...
3. 项目中设置了最大执行步数，用来避免 Agent 因反复调用工具而陷入无限循环。
